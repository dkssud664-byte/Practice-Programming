    ...
    public string[] solution(string[] quiz) {
        ...
        for(int i = 0; i < quiz.Length; i++)
        {
            ...
            for(int j = 0; j < chars.Length; j++)
            {
                ...
                if (chars[j] == '-' && string.IsNullOrEmpty(str))
                {
                    str += '-';
                    continue;
                }

                if (char.IsNumber(chars[j]) && !isLast)
                {
                    str += chars[j]; 
                }
                else if (!isLast && (chars[j] == '+' || chars[j] == '-' || chars[j] == '='))
                {
                    if(isSum)
                    {
