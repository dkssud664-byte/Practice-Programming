...
---
**실행 1(성공)**
생각 : 이중배열을 만들어 실제 이미지와 같이 박스가 쌓인다고 가정하고 값을 넣는다.

그 후 자신보다 위에 값이 있으면 return값을 ++ 한다.
...
public class Solution {
    public int solution(int n, int w, int num) {
            int answer = 1;
            int[,] boxs = new int[(int)Math.Ceiling((float)n / w), w];
            bool isRight = true, isFinish = false;
            int BoxNum = 0, findX = 0, findY = 0;
            ...
            for(int i = 0; i < boxs.GetLength(0); i++)
            {
                ...
                {
                    for(int j = boxs.GetLength(1) - 1; j >= 0; j--)
                    {
                        boxs[i, j] = ++BoxNum;
                        if (BoxNum >= n)
                        {
            ...
            while(true)
            {
                if(++findY >= boxs.GetLength(0) )
                {
                    break;
                }

                if(boxs[findY, findX] != 0)
                {
                    answer++;
                }
            }
...
---
**후기**
- 나는 문제의 박스 쌓기와 동일한 구조로 배열을 만들어 동일한 방법으로 쌓고 실제로 계산을 했는데
- 역시 다른 풀이 방법을 보니 수학적으로 계산해 접근하였다
