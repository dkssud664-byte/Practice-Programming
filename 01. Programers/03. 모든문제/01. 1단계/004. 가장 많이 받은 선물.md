...
---
**실행 1(성공)**
생각 : 서로의 선물 기록 내역을 만들고 내역을 바탕으로 선물지수를 만들고 두 기록을 가지고 선물을 누가 받을지 판단한다.
...
public class Solution {
    public int solution(string[] friends, string[] gifts) {
        ...
        //선물 주고 받은 기록
        List<KeyValuePair<string, Dictionary<string, int>>> giftTable = new List<KeyValuePair<string, Dictionary<string, int>>>();

        //선물 지수
        int[] giftFuture = new int[friends.Length];
        ...
        {
            giftTable.Add(new KeyValuePair<string, Dictionary<string, int>>
                (friends[i], new Dictionary<string, int>()));

            for(int j = 0; j < friends.Length; j++)
            {
                giftTable[i].Value.Add(friends[j], 0);
            }
        }
        ...
        for (int i = 0; i < giftTable.Count; i++)
        {
            ...
            {
                string name = gifts[j].Split(' ')[0];
                string target = gifts[j].Split(' ')[1];

                if (giftTable[i].Key == name)
                {
                    giftTable[i].Value[target]++;
                }
            }
        }
        ...
        for(int i = 0; i < giftTable.Count; i++)
        {
            for(int j = 0; j < friends.Length; j++)
            {
                if (giftTable[i].Key != friends[j])
                {
                    //준 선물
                    giftFuture[i] += giftTable[i].Value[friends[j]];

                    //받은 선물
                    giftFuture[j] -= giftTable[i].Value[friends[j]];
                }
            }
        }
        ...
        for(int i = 0; i < giftTable.Count; i++)
        {
            ...
            {
                //선물 개수로 판단
                if (giftTable[i].Key != friends[j] && giftTable[i].Value[friends[j]] > giftTable[j].Value[friends[i]] )
                {
                    giftCount[i]++;
                }
                //선물 지수로 판단
                else if(giftTable[i].Key != friends[j] && giftTable[i].Value[friends[j]] == giftTable[j].Value[friends[i]])
                {
                    if (giftFuture[i] > giftFuture[j])
                    {
                        giftCount[i]++; 
                    }
                }
...
---
**후기**
KeyValuePair를 자주 사용하지 않아 값을 꺼내쓰는게 미숙했다