...
---
**실행 1(실패)**
생각 : 문자열로 된 아규먼트들을 int로 변환하고 마감기한만큼 더해 현재날짜와 비교한다.

결과 : 6~19 케이스 실패

실패 이유 : 더한 월 수의 하루전까지만 가능한데 1일에서 하루 전이면 월이 바뀌는 경우를 생각 안했다.
...
public class Solution {
    public int[] solution(string today, string[] terms, string[] privacies) {
            ...
            Dictionary<char, int> dictionaryTerm = new Dictionary<char, int>();
            int[,] period = new int[privacies.Length, 4];
            int[] currentDay;

            //today를 인트로 변환
            currentDay = ConversionDate(today);
            ...
            foreach(string term in terms)
            {
                ...
                dictionaryTerm[term[0]] = int.Parse(temp);
            }

            int indexY = 0;
            int indexX = 0;
            //privacies를 속성별로 나누기
            foreach (string s in privacies)
            {


                int[] temp = ConversionDate(s);

                foreach (int i in temp)
                {
                    period[indexY, indexX++] = i;
                }
            ...
            //terms만큼 개인정보에 더하기
            Deadline(dictionaryTerm, period);

            for(int i = 0; i < period.GetLength(0); i++)
            {
                //년도가 같거나 크면
                if (currentDay[0] >= period[i, 0])
                {
                    //년도가 크면 결과에 추가
                    if (currentDay[0] > period[i, 0])
                    {
                    ...
                    //년도가 같으면 월을 비교
                    //월이 크거나 같으면
                    else if (currentDay[1] >= period[i, 1])
                    {
                        //월이 크다면 결과에 추가
                        if(currentDay[1] > period[i, 1])
                        {
                            list.Add(i + 1);
                            continue;
                        }
                        //월이 같으면 일을 비교
                        //일이 크면
                        else if (currentDay[2] >= period[i, 2])
                        {
                            list.Add(i + 1);
                            continue;
                        }
        ...
        public void Deadline(Dictionary<char, int> terms, int[,] privacie)
        {
            for(int i = 0; i < privacie.GetLength(0); i++)
            {
                privacie[i, 1] += terms[(char)privacie[i, 3]];
    ...
    public int[] solution(string today, string[] terms, string[] privacies) {
        ...
        public void Deadline(Dictionary<char, int> terms, int[,] privacie)
        {
            for(int i = 0; i < privacie.GetLength(0); i++)
            {
                ...
                if (privacie[i, 1] > 12)
                {
                    int year = privacie[i, 1] / 12;
                    if (privacie[i, 1] % 12 == 0)
                    {
                        year--;
                        privacie[i, 1] = 12;
                    }
...
---
**후기**
반례를 찾으려 질문하기와 gpt를 활용했는데 반례를 찾는데만 2시간이 넘고 결국 못 찾았아 gpt의 환각이 맞는 답을 자꾸 틀리다고 케이스를 줘 일일히 다 입력해 보느라 시간이 오히려 더 걸렸다.

다음에는 로직을 다시 살펴보고 직접 반례를 찾는 연습을 해야겠다.