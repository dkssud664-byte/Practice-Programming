...
---
**실행 1(성공)**
생각 : 처음 드래그 위치를 알기 위해 x,y의 최소값을 알고 마지막 드래그 위치를 알기위해 x,y의 최대값을 알아낸다.
...
public class Solution {
    public int[] solution(string[] wallpaper) {
        //minY, minX, maxY, maxX
         ...
         int[,] ractangle = new int[wallpaper.Length, wallpaper[0].Length];


         for(int i = 0; i < ractangle.GetLength(0); i++)
         {
             for(int j = 0; j < ractangle.GetLength(1); j++)
             {
                 if (wallpaper[i][j] == '.')
                 {
                     ractangle[i, j] = 0;
         ...
         for (int i = 0; i < ractangle.GetLength(0); i++)
         {
             ...
             {
                 //최소 y
                 if (ractangle[i, j] == 1 && answer[0] > i)
                 {
                     answer[0] = i;
                 }

                 //최소 x
                 if (ractangle[i, j] == 1 && answer[1] > j)
                 {
                     answer[1] = j;
                 }

                 //최대 y
                 if (ractangle[i, j] == 1 && answer[2] < i)
                 {
                     answer[2] = i;
                 }

                 //최대 x
                 if (ractangle[i, j] == 1 && answer[3] < j)
                 {
                     answer[3] = j;
                 }
             }
...
---
**후기**
파일의 위치를 점으로 생각하고 풀어서 최대값을 구할때 헷갈렸다.
