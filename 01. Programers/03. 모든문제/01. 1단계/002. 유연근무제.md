...
---
**실행 1(실패)**
생각 : 각 직원의 출근시간 10분을 계산해서 계산한 값과 타임로그를 비교해 평일에 지각을 안했으면 answer++한다.

결과 : 
테스트 케이스 1 : 실행한 결괏값 2이 기댓값 3과 다릅니다.
테스트 케이스 2 : 실행한 결괏값 3이 기댓값 2과 다릅니다.

실패 이유 : 시간, 분을 넣을때 조건이 잘 못 되었다
...
public class Solution {
    public int solution(int[] schedules, int[,] timelogs, int startday)     {
            int answer = 0;
            int[,] time = new int[schedules.Length, 3]; //[0]시, [1]분, [2]비교값으로 변환
            int week = startday;    //토, 일 예외처리

            //직원 수 만큼 반복
            for(int i = 0; i < schedules.Length; i++)
            {
                string strTime = schedules[i].ToString();
                string strTemp = "";

                //시간, 분 넣기
                for (int k = 0; k < strTime.Length; k++)
                {
                    if (strTime.Length - 3 == i)
                    {
                        strTemp = "";
                    }
                    ...
                    {
                        strTemp += strTime[k];
                        time[i, 0] = int.Parse(strTemp);
                    }
                    else
                    {
                        strTemp += strTime[k];
                        time[i, 1] = int.Parse(strTemp);
                    }
                }
            }
            ...
            for(int i = 0; i < time.GetLength(0); i++)
            {
                ...
                time[i, 2] = time[i, 0] * 100 + time[i, 1];
            }

            //지각 체크
            for(int i = 0; i < schedules.Length; i++)
            {
                bool isNotLate = true;  //지각 체크

                for(int j = 0; j < timelogs.GetLength(0); j++)
                {
                    if(time[i, 2] < timelogs[i, j] && week % 7 != 6 && week % 7 != 7)
                    {
                        isNotLate = false;
                    }
                }
...
---
**실행 2(실패)**
생각 : 실패1의 조건을 수정하고 다시 실행해봤다

결과 : 
테스트 케이스 1 : 성공
테스트 케이스 2 :   
실행한 결괏값 1이 기댓값 2과 다릅니다.
...
public class Solution {
    public int solution(int[] schedules, int[,] timelogs, int startday)     {
            ...
            for(int i = 0; i < schedules.Length; i++)
            {
                ...
                for (int j = 0; j < timelogs.GetLength(0); j++)
                {
                    if(time[i, 2] > timelogs[i, j] && week % 7 != 6 && week % 7 != 0)
                    {
                        isNotLate = false;
                        break;
                    }
...
---
**실행 3(실패)**
생각 : 테스트 케이스 2를 직접 넣어보고 디버그를 실행하며 왜 그런지 알아보자
결과 : 
테스트 케이스 1 : 성공
테스트 케이스 2 :  성공

케이스 1,2,3,4,6번을 제외한 나머지 케이스 실패, 런타임 에러
...
---
**실행 4(성공)**
생각 : 실패 케이스가 뭐가있을까 고민하고 케이스에 대해 질문했더니 로직에 오류가 있었다.

출근시간과 타임로그의 요일 전체를 비교해야하는데 GetLength(0)으로 비교를 하고 있어 직원 수 만큼만 비교 중이었다.
...
---
**후기**
꼼꼼하지 못해 금방 풀 문제를 여러번 디버그, 수정했다.
다음에는 더 디테일한 부분까지 신경쓰자.