...
---
**실행 1(실패)**
생각 : while문으로 각 각의 버튼을 돌려 먼저나온 횟수를 결과에 더해준다.
...
public class Solution {
    public int[] solution(string[] keymap, string[] targets) {
        ...
        for(int i = 0; i < targets.Length; i++)
        {
            ...
            if (answer[i] == 0 || count != targets[i].Length)
            {
                answer[i] = -1;
            }
        }
...
---
**후기**
반례를 질문하기를 통해 바로보지 말고 좀 더 생각하는 습관을 들여야 겠다.