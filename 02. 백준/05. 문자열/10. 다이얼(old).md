...
{  
    ...
    int numberGroup[8] = { 3,3,3,3,3,4,3,4 };
    int num = 2;
    int idx = 0;
    int time = 0;

    map<char, int> dialMap;

    string strInput;

    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < numberGroup[i]; j++)
        {
            dialMap.insert(pair<char, int>(strAlphabet[idx++], num));
        }
        num++;
    }
    ...
    {
        for (map<char, int>::iterator it = dialMap.begin(); it != dialMap.end(); it++)
        {
